...
describe("GameManagementService", () => {
	...
	beforeEach(() => {
		...
		gameRepository = {
			...
			addPlayerToGame: jest.fn((game: Game, playerId: string) => {
	...
	describe("addPlayerToGame", () => {
		...
		it("ajoute un joueur existant au game et l'enregistre", async () => {
			...
			gameRepository.addPlayerToGame = jest.fn(() =>
				Promise.resolve({ id: "gp1", game_id: "g1", player_id: "p2" }),
			);
		...
		it("crée un joueur inconnu, l'ajoute au game et enregistre", async () => {
			...
			playerRepository.createPlayer = jest.fn(() =>
				Promise.resolve(createdPlayer),
			);
			playerRepository.findById = jest.fn(() => Promise.resolve(createdPlayer));
			gameRepository.addPlayerToGame = jest.fn(() =>
				Promise.resolve({ id: "gp2", game_id: "g1", player_id: "p3" }),
			);
			eventEmitter.emit = jest.fn();
		...
		it("renvoie une erreur si le joueur tente de rejoindre deux fois la même partie", async () => {
			...
			// Then
			await expect(service.addPlayerToGame("g1", "Bob")).rejects.toThrow(
