...
import {CivilianCard, ScienceCard} from "../../domain/cards/card.value-object";
...
export class EndGameUsecase {
    ...
    async execute(gameId: string): Promise<void> {
        ...
        for (const player of game.players) {
            ...
            player.victoryPoints = wonderPoints + coinPoints + militaryPoints + civilianPoints + commercialPoints + sciencePoints + guildPoints;
        }
    ...
    private calculateCivilianPoints(player: Player): number {
        const civilianPoints = player.board
            ...
            .reduce((sum, card: CivilianCard) => sum + (card.civilizationPoints || 0), 0);
    ...
    private calculateSciencePoints(player: Player): number {
        const scienceCards: ScienceCard[] = player.board.filter(card => card.type === CardType.SCIENCE) as ScienceCard[];
        const symbolCounts = {
