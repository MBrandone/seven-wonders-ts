...
export class Deck {
    ...
    getCardsForAge(age: 1 | 2 | 3, playerCount: number): Card[] {
        ...
        const guildsCards = this.allCards.filter(card => card.type === CardType.GUILD);
        const randomGuildCardsToAdd = this.getRandomGuildCards(guildsCards, playerCount);
    ...
    getRandomGuildCards(guildsCards: Card[], playerCount: number) {
        return guildsCards.sort(() => Math.random() - 0.5).slice(0, playerCount + 2);
    }
} 