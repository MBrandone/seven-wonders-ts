...
let GameRepository = class GameRepository {
    ...
    async findById(id) {
        ...
        return game_entity_1.Game.hydrate(gameRow.id, gameRow.created_at, gameRow.max_players, players.map(p => p.player_id), gameRow.status);
    }
    ...
    async findWaitingGames() {
        ...
        for (const row of gameRows) {
            ...
            games.push(game_entity_1.Game.hydrate(row.id, row.created_at, row.max_players, players.map(p => p.player_id), row.status));
        }
        return games;
    }
