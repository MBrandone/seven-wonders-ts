...
export class SevenWondersGame {
  ...
  nextAge() {
    ...
    for (let i = 0; i < howManyPlayers; i++) {
      ...
      const leftNeighbour = this.players[(i - 1 + howManyPlayers) % howManyPlayers];
    ...
    if (this.currentAge < 3) {
      ...
      const shuffledCardsToAssign = this.deck.getCardsForAge(this.currentAge, howManyPlayers).sort(() => Math.random() - 0.5);
      this.players.forEach((player, index) => {
        const cardsToGive = shuffledCardsToAssign.slice(index * 7, (index + 1) * 7);
        player.takeCards(cardsToGive);
      });
